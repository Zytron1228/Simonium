...
class SimonActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun gameSounds() {
        if (soundPool == null) {
            ...
            soundPool!!.setOnLoadCompleteListener {_, sampleID, _ ->
                ...
                if (!simonTimerTaskCompleted && simonInstanceSaved) litCells()
                if (!simonInstanceSaved && sampleID == simonSoundID[3]) litCells()
            }
    ...
    private fun simonCellClicked(cellNumber: Int, cell: SimonCellType) {
        if (simonTimerForClick != null) simonTimerForClick!!.cancel() 
        cell.setOn()  
        if (simonPrevSoundStreamID != 0) soundPool!!.stop(simonPrevSoundStreamID) 
        if (simonSoundOn) {
            simonPrevSoundStreamID = soundPool!!.play(
                simonSoundID[cellNumber],
                simonSoundVolume, simonSoundVolume, 1, 0, 1f
            )
        }
        startTimer(cell)
